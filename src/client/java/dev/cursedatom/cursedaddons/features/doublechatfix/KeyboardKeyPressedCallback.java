 ...
 *
 * <p>Upon return:
 * <ul><li>SUCCESS/CONSUME cancels further listener processing and forwards the key press to be processed by the client.
...
public interface KeyboardKeyPressedCallback {
    Event<KeyboardKeyPressedCallback> EVENT = EventFactory.createArrayBacked(KeyboardKeyPressedCallback.class,
            listeners -> (window, key, scancode, i, modifiers) -> {
                for (KeyboardKeyPressedCallback listener : listeners) {
                    InteractionResult result = listener.onKeyPressed(window, key, scancode, i, modifiers);
     ...
     */
    InteractionResult onKeyPressed(long window, int key, int scancode, int action, int modifiers);
}
